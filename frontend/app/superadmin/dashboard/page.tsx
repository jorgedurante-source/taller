...
export default function SuperAdminDashboard() {
    ...
    const handleCreateWorkshop = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(err.response?.data?.message || 'Error al crear taller');
        }
    };
    ...
    const handleImpersonate = async (slug: string) => {
        try {
            ...
            alert('Error al conectar con el taller');
        }
    };

    const handleResetAdminPassword = async (slug: string) => {
        if (!adminPassword) return alert('Debes ingresar una contraseña');
    ...
    const handleDeleteWorkshop = async (slug: string) => {
        ...
        try {
            ...
            alert(err.response?.data?.message || 'Error al eliminar taller');
        }
    };
                        ...
                        {filteredWorkshops.map(w => (
                            <div key={w.id} className="bg-white border border-slate-100 rounded-[3rem] p-8 shadow-sm hover:shadow-2xl hover:shadow-slate-200/50 transition-all group relative overflow-hidden">
                                ...
                                <div className="flex items-start gap-4 mb-6">
                                    ...
                                    <div className="pt-1">
                                        <h4 className="text-2xl font-black text-slate-900 tracking-tighter italic uppercase leading-tight line-clamp-1 pr-6">{w.name}</h4>
